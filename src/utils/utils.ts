/**
 * Original: https://github.com/blockchainsllc/in3-contracts/tree/19b23ca9f030b041cd12efddaca960bbf0b09d82/src/utils/utils.js
 */

import {TransactionConfig} from "web3-core";
import * as in3Common from 'in3-common';
import ethUtil from 'ethereumjs-util';
import crypto from 'crypto';

/** creates a random private key and transfers some ether to this address */
export async function createAccount(seed?, eth = in3Common.util.toBN('50000000000000000000')): Promise<{privateKey: string, address: string}> {
    const pkBuffer = seed
        ? seed.startsWith('0x')
...
export async function handleTx(txParams, privatekey) {

    let transactionParams: TransactionConfig = {}
...
export async function signForRegister(url, props, weight, owner, pk) {
...
export const bytes32 = in3Common.serialize.bytes32

export async function signBlock(b, registryId, pk, blockHash) {
...
export async function createConvictHash(blockhash, signer, v, r, s) {
...
export async function increaseTime(web3, secondsToIncrease) {
...
export async function signHash(pk, msgHash) {
...
export async function signWithPrefix(pk, msgHash) {
